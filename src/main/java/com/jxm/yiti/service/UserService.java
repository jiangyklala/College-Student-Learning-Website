...
//            LOG.info("user: {}", userQueryResp.toString());
    ...
    public String getGptTotalInfo() {
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            String ntimes = JedisUtil.getOrZero(jedis, "yt:gpt:ntimes:" + nowTime);
            String ntokens = JedisUtil.getOrZero(jedis, "yt:gpt:ntokens:" + nowTime);
            String vtimes = JedisUtil.getOrZero(jedis, "yt:gpt:vtimes:" + nowTime);
            String vtokens = JedisUtil.getOrZero(jedis, "yt:gpt:vtokens:" + nowTime);

            String iVtimes = JedisUtil.getOrZero(jedis, "yt:gpti:vtimes:" + nowTime);
            res += "当日普通用户: 提问总数: " + ntimes + ", 消耗的总token: " + ntokens + "\n"
                    + "当日会员用户: 提问总数: " + vtimes + ", 消耗的总token: " + vtokens + "\n"
                    + "当日画图使用次数: " + iVtimes + "\n"
                    + "截止到目前普通用户: 总共消耗: 提问次数: " + (Long.parseLong(JedisUtil.getOrZero(jedis, "yt:gpt:ntimes:ntotal")) + Long.parseLong(ntimes))
                    + ", 消耗的总token: " + (Long.parseLong(JedisUtil.getOrZero(jedis, "yt:gpt:ntokens:ntotal")) + Long.parseLong(ntokens)) + "\n"
                    + "截止到目前会员用户: 总共消耗: 提问次数: " + (Long.parseLong(JedisUtil.getOrZero(jedis, "yt:gpt:vtimes:vtotal")) + Long.parseLong(vtimes))
                    + ", 消耗的总token: " + (Long.parseLong(JedisUtil.getOrZero(jedis, "yt:gpt:vtokens:vtotal")) + Long.parseLong(vtokens)) + "\n"
                    + "截止到目前画图使用次数: " + (Long.parseLong(JedisUtil.getOrZero(jedis, "yt:gpti:vtimes:vtotal")) + Long.parseLong(iVtimes));
